...
1. Examine the `pyproject.toml` what dependencies does it currently identify?
   > Currently the toml dependenceis array is empty
   > dependencies = []
...
- What role does OpenCV have in this project?
  > OpenCV’s role in this project is that it can open and read video frame by frame, which can be used to send the frames need to the OCR which can then perform its tasks to transcribe the text that is visible in the given frame.
  >
  > OpenCV can also be used to manipulate the frames before sending to the OCR to make it easier for the OCR to correctly read the frame such as changing the frames grayscale and noise reduction.
- What is the `uv pip` command to install OpenCV?
  > `uv pip install opencv-python`
- What is the URL of this library's git repo?
  > [opencv-python URL](https://github.com/opencv/opencv-python)

1. Add OpenCV to your project using the `uv add` command:

   > `uv add opencv-python

1. Have the dependencies in the `pyproject.toml` changed? If so, how?
   > The dependencies have updated to
   > dependencies = [
   >
   > > "opencv-python>=4.13.0.92",
   > > ]
1. Why did we use `uv add` over `uv pip`?
   > We used uv add over uv pip becuase by using uv add, uv will automatically update the project toml file with its added dependencies
1. The `numpy` library is required for OpenCV. Should you add an explicit requirement for it? Why/Why not?
   > No we do not need to add numpy as an explicit requirement as we do no plan to import numpy in our files and will only be using it through the use of OpenCV
1. Commit the changes so far to git. Use the message `chore: add OpenCV dependency`
1. Go to `preliminary/library_basics.py` and complete the required functionality.
1. Commit your changes with `feat: save video frames`
...
2. What is the URL that lists Python wrappers for Tesseract?
   ...
   > <[Python wrappers](https://tesseract-ocr.github.io/tessdoc/AddOns.html)>
